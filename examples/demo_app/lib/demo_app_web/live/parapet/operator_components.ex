defmodule DemoAppWeb.Parapet.OperatorComponents do
  ...
  def incident_summary(assigns) do
    ...
    <div>
      <div class="flex justify-between items-start mb-4">
        <div>
          <h1 class="text-2xl font-bold text-stone-900 text-balance text-balance"><%= @detail.incident.title %></h1>
          <p class="text-sm text-stone-500 mt-1"><span class="tabular-nums font-mono"><%= @detail.incident.id %></span></p>
        </div>
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
          ...
          <%= if @detail.escalation_summary.time_until_next_escalation do %>
            <div class="rounded-lg ring-1 ring-stone-900/5 shadow-sm bg-white p-3 sm:col-span-2">
              ...
              <dd class="mt-1 text-amber-950"><span class="tabular-nums font-mono"><%= countdown_copy(@detail.escalation_summary.time_until_next_escalation) %></span></dd>
            </div>
          <% end %>
  ...
  def runbook_card(assigns) do
    ~H"""
    <div class="bg-white shadow-sm ring-1 ring-stone-900/5 bg-white rounded-xl p-4 mb-6">
          ...
          <div class="ring-1 ring-stone-900/5 bg-stone-50 rounded-lg p-3">
            <div class="flex justify-between items-start">
              <div class="flex-1">
                <div class="flex items-center gap-2">
                  ...
                  <%= if step.state == :executed do %>
                    <span class="px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded bg-emerald-100 text-emerald-700 ring-1 ring-emerald-200/50">Executed</span>
                  <% end %>
                </div>
              ...
              <div class="ml-4">
                <%= case step.state do %>
                  <% :previewable -> %>
                    <button phx-click="preview_mitigation" phx-value-step={step.id} phx-value-incident_id={@detail.incident.id} class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-lg text-sm flex items-center justify-center transition-transform duration-100 ease-out active:scale-[0.96] min-h-[40px]">
                      Preview
                    </button>
                  <% :executable -> %>
                     <button phx-click="preview_mitigation" phx-value-step={step.id} phx-value-incident_id={@detail.incident.id} class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-lg text-sm flex items-center justify-center transition-transform duration-100 ease-out active:scale-[0.96] min-h-[40px]">
        ...
        <div class="p-4">
          ...
          <div class="flex gap-2">
            <button 
              ...
              class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-lg text-sm shadow-sm flex items-center justify-center transition-transform duration-100 ease-out active:scale-[0.96] min-h-[40px]"
            >
              Confirm Recovery
            </button>
          </div>
  ...
  def action_rail(assigns) do
    ...
    <div class="flex flex-col gap-4">
      <%= if @detail.incident.state == "open" do %>
        <div class="bg-white shadow-sm ring-1 ring-stone-900/5 bg-white rounded-xl p-4">
          <h4 class="text-sm font-medium text-stone-900 mb-2">Acknowledge</h4>
          <p class="text-xs text-stone-500 mb-3">Take ownership of this incident.</p>
          <button phx-click="acknowledge" phx-value-id={@detail.incident.id} class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-lg text-sm flex items-center justify-center transition-transform duration-100 ease-out active:scale-[0.96] min-h-[40px]">
      ...
      <div class="bg-white shadow-sm ring-1 ring-stone-900/5 bg-white rounded-xl p-4">
        ...
        <p class="text-xs text-stone-500 mb-3">Use bounded controls only after reviewing current status and the canonical timeline.</p>
        <%= if escalation_controls_enabled?(@detail.incident) do %>
          <button phx-click="trigger_next_escalation" phx-value-id={@detail.incident.id} class="w-full bg-amber-600 hover:bg-amber-700 text-white font-medium py-2 px-4 rounded-lg text-sm flex items-center justify-center transition-transform duration-100 ease-out active:scale-[0.96] min-h-[40px] mb-2">
            Trigger Next Escalation
          </button>
          <button phx-click="suppress_pending_escalation" phx-value-id={@detail.incident.id} phx-value-minutes="30" class="w-full bg-white ring-1 ring-amber-300 hover:bg-amber-50 text-amber-900 font-medium py-2 px-4 rounded-lg text-sm flex items-center justify-center transition-transform duration-100 ease-out active:scale-[0.96] min-h-[40px]">
      ...
      <div class="bg-white shadow-sm ring-1 ring-stone-900/5 bg-white rounded-xl p-4">
        ...
        <p class="text-xs text-stone-500 mb-3">Mark incident as resolved.</p>
        <button phx-click="resolve" phx-value-id={@detail.incident.id} class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-medium py-2 px-4 rounded-lg text-sm flex items-center justify-center transition-transform duration-100 ease-out active:scale-[0.96] min-h-[40px]">
          Resolve Incident
        </button>
      </div>
    </div>
  ...
  def action_item_card(assigns) do
    ...
    <div class="bg-white shadow-sm ring-1 ring-stone-900/5 bg-white rounded-xl p-4">
      ...
      <%= if @url do %>
        <a href={@url} target="_blank" rel="noopener noreferrer" class="w-full flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-lg text-sm transition-transform duration-100 ease-out active:scale-[0.96] min-h-[40px]">
          Review in UI &nearr;
        </a>
      <% else %>
  ...
  def critical_journeys(assigns) do
    ~H"""
    <div class="bg-white shadow-sm ring-1 ring-stone-900/5 bg-white rounded-xl p-4 mb-4">
