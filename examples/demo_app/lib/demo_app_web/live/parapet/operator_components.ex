defmodule DemoAppWeb.Parapet.OperatorComponents do
      ...
      <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <%= if Enum.empty?(@items) do %>
          <div class="rounded-xl border border-dashed border-[color:var(--parapet-border)] bg-white/70 p-6 text-center shadow-sm">
            ...
            <p class="text-sm font-semibold" style="color: var(--parapet-text);">No pending action items</p>
            <p class="mx-auto mt-2 max-w-md text-sm" style="color: var(--parapet-text-muted);">Recovery work appears here when an incident has a concrete item to inspect or resolve.</p>
          </div>
        <% end %>
      </div>
    </section>
  ...
  def incident_list(assigns) do
    ...
    <div class="divide-y divide-stone-200">
      ...
      <%= if Enum.empty?(@incidents) do %>
        <div class="mx-4 my-6 rounded-xl border border-dashed border-[color:var(--parapet-border)] bg-white/70 p-6 text-center shadow-sm">
          <svg aria-hidden="true" class="mx-auto mb-3 h-8 w-8" style="color: var(--parapet-text-muted);"
               fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
            <path stroke-linecap="round" stroke-linejoin="round"
          ...
          <%= if @page_mode == :history do %>
            <p class="text-sm font-semibold" style="color: var(--parapet-text);">No resolved incidents yet</p>
            <p class="mx-auto mt-2 max-w-md text-sm" style="color: var(--parapet-text-muted);">
              Resolved incidents land here with their final timeline and retrospective. Active work stays in the response queue until it closes.
            </p>
            <.link navigate={operator_path(@operator_base_path)} class="mt-4 inline-block text-sm font-medium underline" style="color: var(--parapet-accent);">
              Back to active response
            </.link>
          <% else %>
            <p class="text-sm font-semibold" style="color: var(--parapet-text);">No active incidents</p>
            <p class="mx-auto mt-2 max-w-md text-sm" style="color: var(--parapet-text-muted);">
            ...
            <.link navigate={operator_path(@operator_base_path, :history)} class="mt-4 inline-block text-sm font-medium underline" style="color: var(--parapet-accent);">
  ...
  attr(:socket_connected, :boolean, required: true)

  @doc """
  Uniform disconnected-load skeleton (D-09). Renders three matched `h-16` rows under
  `aria-live="polite"` + `aria-busy` keyed on `@socket_connected`, so the Actions and
  History lists share the Response queue's zero-layout-shift loading affordance. The
  caller wraps its real list in the `aria-busy=false` branch; this component is
  skeleton-or-nothing only (no literal "Loading" text — ARIA carries the state).
  """
        ...
        <div class="shadow-sm ring-1 ring-stone-900/5 bg-white rounded-xl p-4">
          ...
          <div class="flex flex-col gap-2">
            <%= if trace_id = Map.get(@detail.incident, :trace_id) do %>
              ...
              <a href={url} class="po-link flex min-w-0 items-center gap-1 text-sm hover:underline" target="_blank" rel="noopener noreferrer">
                ...
                <span class="min-w-0">Trace: <span class="break-all min-w-0 tabular-nums font-mono"><%= trace_id %></span></span> &nearr;
              </a>
            <% end %>
                ...
                <div class="relative flex space-x-4">
                  ...
                  <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1">
                    <div>
                      ...
                      <p class="text-sm md:text-[15px] text-stone-600 mt-1">
          ...
          <%= for entry <- @entries do %>
            ...
            <div class="flex justify-between items-start bg-white ring-1 ring-stone-900/5 rounded-lg p-3 shadow-sm">
              <div class="flex items-start gap-3">
                ...
                <div>
                  <p class="text-sm font-medium text-stone-900">
                    Flag <span class="font-mono text-xs bg-stone-100 px-1.5 py-0.5 rounded border border-stone-200"><span class="break-all min-w-0 tabular-nums font-mono"><%= flag %></span></span> updated
                  </p>
              ...
              <div class="flex flex-col items-end gap-1">
                <span class="po-chip po-chip-info px-2 py-0.5 rounded text-xs">
                  <%= inspect(scope) %>
                </span>
      ...
      <section class={surface_class(:compact_card)} aria-label="Incident retrospective">
        ...
        <div class="space-y-3 rounded-lg bg-stone-50 p-4 ring-1 ring-stone-900/5">
          <%= for block <- retrospective_blocks(@retrospective) do %>
            <%= case block do %>
              <% {:heading, level, text} when level <= 2 -> %>
                <h4 class="text-base font-semibold text-stone-950"><%= text %></h4>
              <% {:heading, _level, text} -> %>
                <h4 class="text-sm font-semibold text-stone-800"><%= text %></h4>
              <% {:list, items} -> %>
                <ul class="list-disc space-y-1 pl-5 text-sm md:text-[15px] leading-6 text-stone-700">
                  <%= for item <- items do %>
                    <li><%= item %></li>
                  <% end %>
                </ul>
              <% {:paragraph, text} -> %>
                <p class="text-sm md:text-[15px] leading-6 text-stone-700"><%= text %></p>
            <% end %>
          <% end %>
        </div>
      </section>
  ...
  def runbook_card(assigns) do
    ...
    <div class={surface_class(:runbook_card)}>
      ...
      <p class="text-sm md:text-[15px] text-stone-600 mb-4">
            ...
            <div class="flex justify-between items-start">
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  <h4 class="break-words text-sm font-medium text-stone-900"><%= step.label %></h4>
                  <%= if step.state == :executed do %>
                    <span class={chip_class(:execution, :executed)}>Executed</span>
                  <% end %>
                </div>
                <p class="break-words text-xs text-stone-500 mt-1"><%= step.description %></p>
        ...
        <div class="p-4">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            ...
            <div class="min-w-0">
              ...
              <p class="text-xl font-semibold tabular-nums text-stone-900 min-w-0 break-words"><%= preview.data["count"] %></p>
            </div>
          </div>
          ...
          <div class="flex flex-col gap-3">
            <p class="po-guidance rounded-lg px-3 py-2 text-xs ring-1">
            ...
            <button 
              ...
              class={["w-full", control_class(:recovery)]}
            >
              Confirm Recovery
            </button>
          </div>
  ...
  def action_item_card(assigns) do
    ...
    <div class={surface_class(:action_card)}>
      <div class="flex justify-between items-start mb-2 min-w-0">
        <h4 class="min-w-0 break-words text-sm font-medium text-stone-900"><%= @item.title || "Action Item" %></h4>
        <div class="flex shrink-0 items-center gap-2">
          <%!-- D-07: risk chip — color + icon + label, never color alone (WCAG 1.4.1) --%>
          <span class={["po-chip rounded px-1.5 py-0.5 text-xs font-semibold uppercase tracking-wide flex items-center gap-1", risk_chip_class(action_item_risk(Map.get(@item, :kind)))]}>
            <%= case action_item_risk(Map.get(@item, :kind)) do %>
              <% :danger -> %>
                <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" /></svg>
              <% :warning -> %>
                <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" /></svg>
              <% :info -> %>
                <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" /></svg>
              <% _ -> %>
                <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg>
            <% end %>
            <%= risk_label(action_item_risk(Map.get(@item, :kind))) %>
          </span>
          <%!-- D-08: single status chip — audit outcome derived honestly from state, color via state_color (no redundant raw-state pill) --%>
          <span class={["rounded px-2 py-0.5 text-xs font-semibold uppercase tracking-wide", state_color(@item.state)]}>
            <%= audit_outcome_label(@item.state) %>
          </span>
        </div>
      </div>
      <p class="min-w-0 break-all text-xs text-stone-500 mb-3"><%= @item.integration %>:<span class="break-all font-mono"><%= @item.external_id %></span></p>
  ...
  # D-07: risk tier from ActionItem.kind — component layer only, no schema change
  ...
  # D-08: audit outcome derived from state — no "failed" state fabricated (open → "Pending" only)
  ...
  defp retrospective_blocks(markdown) when is_binary(markdown) do
    ...
    |> Enum.reduce([], fn line, acc ->
      cond do
        heading = retrospective_heading(line) ->
          [heading | acc]

        bullet = retrospective_bullet(line) ->
          case acc do
            [{:list, items} | rest] -> [{:list, items ++ [bullet]} | rest]
            _ -> [{:list, [bullet]} | acc]
          end
  ...
  defp retrospective_heading("#### " <> rest), do: {:heading, 4, String.trim(rest)}
  defp retrospective_heading("### " <> rest), do: {:heading, 3, String.trim(rest)}
  defp retrospective_heading("## " <> rest), do: {:heading, 2, String.trim(rest)}
  defp retrospective_heading("# " <> rest), do: {:heading, 1, String.trim(rest)}
  defp retrospective_heading(_), do: nil

  defp retrospective_bullet("- " <> rest), do: String.trim(rest)
  defp retrospective_bullet("* " <> rest), do: String.trim(rest)
  defp retrospective_bullet(_), do: nil

  defp readable_datetime(%DateTime{} = datetime) do
    Calendar.strftime(datetime, "%b %d, %Y, %H:%M UTC")
  end

  defp readable_datetime(%NaiveDateTime{} = datetime) do
    Calendar.strftime(datetime, "%b %d, %Y, %H:%M UTC")
  end
  ...
  defp exact_datetime(%NaiveDateTime{} = datetime), do: NaiveDateTime.to_iso8601(datetime)
