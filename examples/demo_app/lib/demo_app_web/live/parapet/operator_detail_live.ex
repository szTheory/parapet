  ...
  def handle_event("suppress_pending_escalation", %{"id" => id, "minutes" => minutes}, socket) do
    ...
    with {minutes, ""} <- Integer.parse(minutes),
      ...
      case Parapet.Operator.suppress_pending_escalation(incident, suppress_until, payload) do
        ...
        {:error, reason} ->
          {:noreply,
           put_flash(socket, :error, "Failed to suppress escalation: #{inspect(reason)}")}
      end
    else
      _ ->
    ...
    <div class="parapet-ui antialiased flex min-h-screen w-full max-w-full flex-col overflow-x-hidden bg-stone-100 text-stone-900">
      ...
      <div class="border-b border-stone-200 bg-white px-4 py-3 md:px-6">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <.link navigate={detail_back_path(@operator_base_path, @incident)} class="inline-flex min-h-[40px] items-center rounded-lg text-sm font-semibold text-teal-800 underline decoration-stone-300 underline-offset-4 hover:text-teal-950">
