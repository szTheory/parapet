  ...
  describe "metrics/0" do
    test "defines prometheus metrics safely" do
      ...
      assert Enum.any?(metrics, fn metric ->
               metric.name == [:parapet, :probe, :run, :duration, :ms]
             end)
  ...
  describe "handle_event/4" do
    test "translates duration to ms and emits unified event" do
      ...
      Probe.handle_event(
        ...
        %{duration: duration},
        %{probe: "MyProbe", status: "success"},
        %{}
      )

      assert_receive {^ref, [:parapet, :probe, :run], %{duration_ms: 100},
                      %{probe: "MyProbe", status: "success"}}
    ...
    test "handles exception event without duration" do
      ...
      Probe.handle_event(
        ...
        %{},
        %{probe: "MyProbe", status: "error"},
        %{}
      )

      assert_receive {^ref, [:parapet, :probe, :run], %{duration_ms: 0},
                      %{probe: "MyProbe", status: "error"}}
