defmodule Parapet.SLO.ScoriaEvalTest do
  ...
  describe "new/1" do
    test "raises ArgumentError when missing required properties" do
      assert_raise ArgumentError, ~r/missing required fields for ScoriaEval my_eval: \[:objective, :guardrail, :runbook\]/, fn ->
  ...
  describe "Parapet.SLO.Resolvable" do
    test "to_slo/1 converts ScoriaEval to a standard Parapet.SLO" do
      ...
      assert slo.good_events == "sum(rate(scoria_evaluation_total{guardrail=\"toxicity\", passed=\"true\", model_name=\"gpt-4\"}[window]))"
      assert slo.total_events == "sum(rate(scoria_evaluation_total{guardrail=\"toxicity\", model_name=\"gpt-4\"}[window]))"
    end
  end
end
