...
The workbench remains host-owned and evidence-first: a calm active-response overview for current truth, canonical timeline for chronology, and risky controls only after enough context is visible.
...
trying to demonstrate every state at once.
...
See the [Recovery Actions Guide](recovery-actions.md) for step-by-step authoring instructions, worked examples, and the error semantics for each capability outcome.
