...
Every alert expression the Generator produces includes a denominator guard. For a slice named `web_saas_login_journey` with a `:page` alert class (14.4× multiplier, 5m window) and a 99.9% objective:
...
The 0.0144 threshold comes from the objective: 99.9% → 0.001 error budget × 14.4 multiplier = 0.0144.
...
- `:page` — 5m window, 14.4× multiplier
- `:ticket` — 30m window, 6.0× multiplier
- `:warning` — 6h window, 1.0× multiplier
