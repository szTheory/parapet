...
- [x] **COLLAT-02**: The off-brand `docs/assets/parapet-logo.svg` and `docs/assets/favicon.svg` are replaced with the on-brand winners, `mix.exs` logo/favicon paths are left unchanged (zero-config swap), and `mix docs` renders the new mark without error.
- [x] **COLLAT-03**: A repo-hygiene audit passes — `brandbook/` within the size budget (≤ ~250 KB), zero raster/font binaries, no full-viewBox rectangular background in any logo asset, no off-palette hex in any SVG/CSS, and the git diff scoped to `brandbook/`, the two `docs/assets/*.svg`, and the `mix.exs` doc block.
...
| COLLAT-02 | 43 | Complete |
| COLLAT-03 | 43 | Complete |
