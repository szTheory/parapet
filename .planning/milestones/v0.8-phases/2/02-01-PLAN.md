...
<task type="auto" tdd="false">
  ...
  <action>
    1. In `lib/parapet/runbook.ex`, update the `step/2` macro to include `auto_execute: Keyword.get(unquote(opts), :auto_execute, false)` in the `@steps` map. (Analog: existing DSL definitions in `lib/parapet/runbook.ex`).
    2. In `lib/parapet/operator.ex`, update `execute_runbook_step/3` to inject the actor identity into BOTH `TimelineEntry` and `ToolAudit`. Ensure the `timeline_attrs` payload includes `"actor" => payload.actor` AND the `ToolAudit` generation explicitly captures `actor: payload.actor` so that the system accurately records who performed the action (e.g., the system executor).
    3. Ensure existing tests pass and add specific unit tests for `auto_execute` in `runbook_test.exs` and identity stamping in `operator_test.exs`.
  </action>
  <verify>
    <automated>mix test test/parapet/runbook_test.exs test/parapet/operator_test.exs</automated>
  </verify>
  <done>Runbook DSL supports auto_execute and Operator properly durably stamps BOTH the TimelineEntry and ToolAudit payload with the payload.actor.</done>
</task>
...
<task type="auto" tdd="false">
  ...
  <action>
    1. Create `lib/parapet/automation/executor.ex` as an `Oban.Worker`. Use `queue: :default` and add `unique: [period: 3600, keys: [:incident_id, :step_id]]` to prevent flap-looping. Follow the pattern from the analog `lib/parapet/escalation/worker.ex`.
    ...
    3. In `lib/parapet/spine/alert_processor.ex`, update `process_firing_alert/1`. When a *new* incident is created (`is_nil(existing_incident)`), scan `runbook_data["steps"]` for any step with `auto_execute: true`. For each matching step, use an `Ecto.Multi` transaction pattern (`maybe_enqueue_automation(runbook_data)`) akin to the `maybe_enqueue_escalation` analog to enqueue the `Executor` job via `Oban.insert!`.
  ...
  <done>Automation executor safely calls the Operator API under system identity, and AlertProcessor correctly routes new alerts to it using standard Ecto.Multi patterns.</done>
</task>
...
|-----------|----------|-----------|-------------|-----------------|
| T-02-01 | Repudiation | Operator | mitigate | Ensure TimelineEntry and ToolAudit explicitly log "actor" => payload.actor to prevent ghost automation. |
...
- `Parapet.Operator` test confirms `TimelineEntry` and `ToolAudit` payloads include `actor`.
