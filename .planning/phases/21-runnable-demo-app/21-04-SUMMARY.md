...
**demo + release_gate CI jobs added (postgres service, smoke test, fan-in gate); examples/ confirmed absent from Hex package; getting-started.md links the demo app**

## Performance

- **Duration:** ~1 min
- **Started:** 2026-05-25T16:12:22Z
- **Completed:** 2026-05-25T16:13:28Z
...
**Task 2 (blocking human checkpoint) — required before this plan is complete:**

1. **REQUIRED STATUS CHECK (DEMO-03):** After the CI workflow has run at least once on a PR (so `release_gate` appears as a selectable check name), configure it as a required status check on `main`:
   - GitHub: Settings → Branches → branch protection rule for `main` → "Require status checks to pass before merging" → add `release_gate`
   - Or via CLI: `gh api -X PUT repos/szTheory/parapet/branches/main/protection/required_status_checks -f 'checks[][context]=release_gate'`
   - Verify: `gh api repos/szTheory/parapet/branches/main/protection/required_status_checks --jq '.checks'`

2. **HEX EXCLUSION (DEMO-04):** From the parapet root run `mix hex.build` and confirm no path under `examples/` appears in the "Files" list. (Already confirmed locally — re-verify from a clean state if desired.)

3. **LIVE DEMO (DEMO-01 / DEMO-02):** `cd examples/demo_app && mix setup && mix phx.server`, then open http://localhost:4000/parapet. Confirm the Operator UI loads with seeded incidents in open/investigating/resolved states, timeline entries, a tool audit, and a runbook step with a `warning:` callout. Run `mix assets.build` first if unstyled.

## Next Phase Readiness

- CI gate is wired: once the workflow runs on a PR, `release_gate` can be added to branch protection
- All automated work is committed and verified
- Blocked on human checkpoint (Task 2) for branch protection configuration and live demo confirmation
- Once checkpoint cleared, this plan is fully complete (DEMO-03, DEMO-04 satisfied)
