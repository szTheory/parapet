defmodule Mix.Tasks.Parapet.Gen.Ui do
  ...
  def igniter(igniter) do
    ...
    |> Igniter.copy_template(
      Path.join([:code.priv_dir(:parapet), "templates", "parapet.gen.ui", "operator_live.ex.eex"]),
      Path.join([web_dir, "operator_live.ex"]),
      assigns,
      on_exists: :skip
    )
    |> Igniter.copy_template(
      Path.join([:code.priv_dir(:parapet), "templates", "parapet.gen.ui", "operator_detail_live.ex.eex"]),
      Path.join([web_dir, "operator_detail_live.ex"]),
      assigns,
      on_exists: :skip
    )
    |> Igniter.copy_template(
      Path.join([:code.priv_dir(:parapet), "templates", "parapet.gen.ui", "operator_components.ex.eex"]),
  ...
  defp add_router_guidance(igniter, assigns) do
    template_path = Path.join([:code.priv_dir(:parapet), "templates", "parapet.gen.ui", "router_snippet.ex.eex"])
